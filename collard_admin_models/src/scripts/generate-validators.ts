...
let currentPathNaame = indexOfPath !== -1 ? process.argv[indexOfPath + 1] : modelsPathName;
...
const getExport = (fileName:string) => {
    return `export const validate${capitalizeFirsLetter(fileName)} = makeValidator(schema.$id);`;
}
...
(async () => await generateVals(currentPathNaame, async (dir:string, file:string) => {
    ...
    const functionName = fileName.split('-').map(s => capitalizeFirsLetter(s)).join('');
    const functionString = `${getImports()} \n ${getSchemaImport(dir,a)} ${configureAjv()} ${getMakeValidatorFun()} ${addSchemaToAjv()} ${getExport(functionName)}`;
    fileSystem.writeFileSync(`${dirName}/validators/generated/${fileName}.validator.ts`,functionString )
}))()
